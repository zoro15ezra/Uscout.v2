...
function renderDmList(threads) {
  ...
  threads.forEach((t) => {
    ...
    const name = otherUser.name || t.name || "Player";
    ...
    btn.className =
      "w-full text-left p-3 rounded-lg hover:bg-slate-800 transition duration-150 border border-slate-800 flex items-center gap-3";
    btn.innerHTML = `
      <div class="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-xs">
        ${escapeHtml((name || "P")[0].toUpperCase())}
      </div>
...
async function openChat(chatId, chatName, isDm) {
  ...
  if (titleEl) titleEl.textContent = chatName || "Chat";
  if (subtitleEl)
...
function renderMessages(messages) {
  ...
  messages.forEach((m) => {
    ...
    const time =
      m.timestamp?.seconds != null
        ? formatTime(new Date(m.timestamp.seconds * 1000))
        : "now";
